...
data = {
    "Widget": [
        ...
        ("用当前图的处理链与 ROI，按文件名顺序播放当前文件夹全部图片",
         "Play all images in the current folder using this image's chain and ROI"),
        ("按住查看原图，松开恢复结果", "Hold to view original, release for result"),
        ("正在全部执行…", "Running all…"),
        ("取消", "Cancel"),
        ("正在处理 %1 / %2\n%3", "Processing %1 / %2\n%3"),
        ("完成：成功 %1 张，失败 %2 张。", "Done: %1 succeeded, %2 failed."),
        ("\n（已取消，停留在当前张）", "\n(Canceled; stayed on current image)"),
        ("当前图片所在文件夹下没有找到图片文件",
         "No images found in the current image's folder"),
